...
No Azure SDK required — uses only requests (already in your requirements).
...
For LAUNCHING VMs (launcher.py — future):
    You'll need a Service Principal. Create one:
    ...
    3. App registrations → your app → Certificates & secrets
       → New client secret → copy the VALUE immediately
       AZURE_CLIENT_SECRET  = the secret value

    4. Go to your Subscription → Access control (IAM)
       → Add role assignment → "Contributor" → assign to your app

    5. Go to your Subscription → Overview → copy Subscription ID
       AZURE_SUBSCRIPTION_ID = your subscription ID

    6. Add to your .env / environment:
       AZURE_CLIENT_ID=...
       AZURE_CLIENT_SECRET=...
       AZURE_TENANT_ID=...
    ...
    For pricing only (this file), none of the above are needed.
─────────────────────────────────────────────────────────────────────
"""
...
# "Spot" prices fetched live; these are fallbacks.
...
def _fetch_live_spot_prices(instance_types: list[str]) -> dict[str, float]:
    ...
    try:
        ...
        # Azure spot SKUs end with " Spot"
        # Filter by armRegionName and priceType=Spot
        filter_str = (
        ...
        for item in items:
            # skuName looks like "D4s v3 Spot" → normalize to Standard_D4s_v3
            sku   = item.get("skuName", "").replace(" Spot", "").strip()
            price = float(item.get("retailPrice", 0))
            # Match against our catalog by normalizing spaces/underscores
